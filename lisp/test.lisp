...
(mat4:print (mat4:rotation 1.0 (vec3:new 1 0 0)))
(defvar m2 (mat4:rotation 1.0 (vec3:new 1 0 0)))
;(set m2 (mat4:scale 2.0 2.0 2.0))
(dotimes (i 4)
  (dotimes (j 4)
	 (let ((s ""))
		(dotimes (k 4)
		  (unless (or (eq (mat4:get m2 k j) 0.0) (eq (mat4:get m2 k j) 1.0))
			 (set s (concat s (if (eq s "") "" " + ") "m1_" i "_" k " * " "m2_" k "_" j))))
		(unless (eq s "")
		  (println (concat "r_" i "_" j " = " s))))))
(defvar m2 (mat4:rotation 1.0 (vec3:new 1 0 0)))
;(set m2 (mat4:scale 2.0 2.0 2.0))
(dotimes (i 4)
  (dotimes (j 4)
	 (let ((s ""))
		(dotimes (k 4)
		  (unless (or (eq (mat4:get m2 k j) 0.0) (eq (mat4:get m2 k j) 1.0))
			 (set s (concat s "(* " "(th m1 " (+ i (* k 4)) ") " "(th m2 " (+ k (* 4 j)) "))"))))
		(unless (eq s "")
		  (println (concat "(set (th r " (+ i (* 4 j)) ") (+ " s "))"))))))
(dotimes (i 10)
  (mat4:scale m2 1.0 1.0 2.0))
(let ((axis-vector (vec3:new 1 0 0)))
  (dotimes (i 30000000)
	 (mat4:rotate m2 1.0 axis-vector)))
